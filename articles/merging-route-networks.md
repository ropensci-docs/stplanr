...
We pre-processed the input simple geometry to make it even simpler as
shown below.
...
We can more reduce the minimum segment length to ensure fewer NA values
in the outputs:
...
As shown in the results, some sideroad values have unrealistically high
values:
...
The good news: the number of NAs is down to only 21 compared with the
previous 100+. Bad news: sideroads have been assigned values from the
main roads.
